...
**✅ 实验 C 已执行（2026-09-11），三重独立证据确证 μs**：

1. **工具链不判定单位**：`cannjudge-submit/SKILL.md:104` 明文写"time 单位以页面/评测说明为准"——
   工具只是透传平台原值。这解释了 ms/μs 混乱的来源：**没有任何一处代码做过单位判定**。
2. **物理反推（决定性）**：Case5 数据量 50KB（x 16KB + w 32KB + out 2KB）。
   若单位是 ms → 耗时 8.2ms → 带宽 **6.2 MB/s**，比任何 NPU 慢 5 个数量级。单位必是 μs。
3. **启动开销量级**：若单位是 ms，则 Case1（仅 16 个元素）耗时 2.94ms——同样荒谬。
