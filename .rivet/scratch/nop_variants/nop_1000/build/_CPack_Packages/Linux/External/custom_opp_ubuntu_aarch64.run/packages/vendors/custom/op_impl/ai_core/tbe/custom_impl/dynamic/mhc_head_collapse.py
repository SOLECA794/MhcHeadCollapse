...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'hc_base', 'hc_scale']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm, eps_hc):
    __inputs__ = []
    for arg in [x_in__, weight_in__, hc_base_in__, hc_scale_in__]:
        if arg != None:
...
@tbe_register.register_operator("MhcHeadCollapse", trans_bool_to_s8=False)
...
def mhc_head_collapse(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm=1e-06, eps_hc=1e-06, kernel_name="mhc_head_collapse", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm, eps_hc)
    ...
    origin_func_name = "mhc_head_collapse"
    ascendc_src_dir_ex = "mhc_head_collapse"
    ascendc_src_dir = "mhc_head_collapse"
    ...
    if src_file != "":
        ...
        ascendc_src_file = "mhc_head_collapse.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MhcHeadCollapse, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MhcHeadCollapse"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, hc_base_in__, hc_scale_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm=1e-06, eps_hc=1e-06, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm, eps_hc)
    result = check_op_cap("op_select_format", "MhcHeadCollapse", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm=1e-06, eps_hc=1e-06, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, hc_base_in__, hc_scale_in__, y_out_, eps_norm, eps_hc)
    result = check_op_cap("get_op_specific_info", "MhcHeadCollapse", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
