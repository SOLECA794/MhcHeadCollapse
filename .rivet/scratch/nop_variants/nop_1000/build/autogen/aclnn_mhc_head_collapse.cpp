...
#include "MhcHeadCollapse_op_resource.h"
...
[[maybe_unused]] uint32_t NNOPBASE_MhcHeadCollapse = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
...
#ifdef ACLNN_WITH_BINARY
    static uint32_t MhcHeadCollapseOpTypeId = op::GenOpTypeId("MhcHeadCollapse", MhcHeadCollapse_RESOURCES);
#endif
    static NnopbaseDfxId dfxId = {0x60000, __func__, false};
    static NnopbaseDfxId tilingId = {0x60000, "aclnnMhcHeadCollapseTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "MhcHeadCollapse";
    ...
    NNOPBASE_ASSERT_NOTNULL_RETVAL(hcBase);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(hcScale);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
...
#ifdef ACLNN_WITH_BINARY
    NnopbaseAddOpTypeId(*executor, MhcHeadCollapseOpTypeId);
#endif
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, x, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, weight, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, hcBase, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, hcScale, 3));
    float tmp0 = static_cast<float>(epsNorm);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&tmp0), sizeof(float), 0, kNnopbaseFloat));
    float tmp1 = static_cast<float>(epsHc);
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "hcBase", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 3, "hcScale", true));
