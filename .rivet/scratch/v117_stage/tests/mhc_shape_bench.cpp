...
int main(int argc, char **argv) {
    ...
    // tensors（8.5 的 aclCreateTensor 9 参签名，与 mhc_correctness 一致）
    aclTensor *x_t = nullptr, *w_t = nullptr, *b_t = nullptr, *s_t = nullptr, *y_t = nullptr;
    {
        ...
        const int64_t x_strides[3] = {kOuter * kNH, kNH, 1};
        const int64_t w_strides[2] = {kNH, 1};
        const int64_t b_strides[1] = {1};
        const int64_t y_strides[3] = {kOuter * kH, kH, 1};
        const aclDataType xdt = use_fp16 ? ACL_FLOAT16 : ACL_FLOAT;
        x_t = aclCreateTensor(x_dims, 3, xdt, x_strides, 0, ACL_FORMAT_ND, x_dims, 3, x_dev);
        w_t = aclCreateTensor(w_dims, 2, ACL_FLOAT, w_strides, 0, ACL_FORMAT_ND, w_dims, 2, w_dev);
        b_t = aclCreateTensor(b_dims, 1, ACL_FLOAT, b_strides, 0, ACL_FORMAT_ND, b_dims, 1, b_dev);
        s_t = aclCreateTensor(b_dims, 1, ACL_FLOAT, b_strides, 0, ACL_FORMAT_ND, b_dims, 1, s_dev);
        y_t = aclCreateTensor(y_dims, 3, xdt, y_strides, 0, ACL_FORMAT_ND, y_dims, 3, y_dev);
    }
    uint64_t ws_size = 0;
