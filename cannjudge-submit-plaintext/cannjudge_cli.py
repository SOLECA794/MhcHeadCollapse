...
def print_submission_result(result: dict):
    ...
    for idx, tc in enumerate(result.get("result", [])):
        ...
        # 平台返回的 time 是裸数字，单位是微秒（μs），非毫秒。
        # 见 FACTS.md F2.1：case1 仅 312B 数据，若按 ms 计则带宽 143.7 KB/s，比任何 NPU 慢数个数量级。
        time_us = tc.get("time", "N/A")
        print(f"  Case {idx + 1}: {tc_status} | precision: {precision} | time: {time_us}us")
