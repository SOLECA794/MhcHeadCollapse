...
rms_inv=11nH∑k=1nHflatk2+εnorm
...
mixes_logitsi=∑k=1nHWi,k⋅flatk,i=1,…,n

1. 将 logits 与 RMS 逆逐元素相乘（广播至 `(…, n)`），得到混合系数 `mixes`：

mixesi=mixes_logitsi⋅rms_inv
...
prei=σ(mixesi⋅sscale+bbase,i)+εhc
...
output=∑i=1nprei⋅streamsi∈R…×H

1. 将 `output` 从 FLOAT32 回退至输入数据类型，作为最终输出 `y`。
