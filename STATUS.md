...
**最后更新**：2026-09-14（会话巅峰：**5 个 case 真实 shape 全部实测钉死**——F3.9）

---

## 当前主线：本地 vs OJ 对拍，测出真实 case shape

用户核心目标（换到本 NPU 环境的原因）：~~反推 5 个 case 的真实 (n, nH, outer)~~
**✅ 已完成（F3.9）**：C1/C2=(4,16,2) C3/C4=(8,512,8) C5=(8,1024,32)。
手段：TilingFunc 断言探测（8 发提交）+ NOP 标定 + 本地矩阵三方交叉。
拟合模型被实测否定（预测 C3/C4∈{1,2} 实测 8）——本地-OST 绝对拟合不可行，
后续优化走"本地 A/B 相对比较 + OJ 直测验证"。
